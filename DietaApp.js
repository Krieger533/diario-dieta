...
// ── MEAL CARD ─────────────────────────────────────────────────────────────────
function RefeicaoCard({ ref: _, refeicao, dayData, onChange }) {
  ...
  function toggleCheck() {
    onChange('checks', refeicao.id, !checked)
  }
  ...
  function setNota(val) {
    onChange('notas', refeicao.id, val)
  }
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      padding: '10px 0', borderBottom: i < refeicao.estrutura.length - 1 ? '1px solid #F7F6F3' : 'none',
                    }}
                  >
                    <div
                    ...
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: itemChecked ? '#aaa' : '#333', textDecoration: itemChecked ? 'line-through' : 'none' }}>
                        {item.nome}
                      </div>
            ...
            <div style={{ marginTop: 12 }}>
              {refeicao.opcoes.map((op, i) => {
                ...
                return (
                  <div
                    key={i}
                    onClick={() => onChange('checks', k, !v)}
                    style={{
                      display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0',
                      borderBottom: i < refeicao.opcoes.length - 1 ? '1px solid #F7F6F3' : 'none',
                      cursor: 'pointer',
                    }}
                  >
                    <div style={{
                      width: 18, height: 18, borderRadius: 4, flexShrink: 0,
                      border: v ? 'none' : '1.5px solid #ccc',
                      background: v ? '#1A7A5E' : '#fff',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <textarea
              ...
              style={{
                ...
                resize: 'vertical', outline: 'none', color: '#444',
...
export default function DietaApp() {
  ...
  function changeDay(dir) {
    ...
    if (newKey > todayKey()) return // não avança além de hoje
    ...
    <div style={{ maxWidth: 600, margin: '0 auto', padding: '20px 14px 60px' }}>
      ...
      {toast && (
        <div style={{
          ...
          background: '#1A7A5E', color: '#fff', fontSize: 14, padding: '10px 22px',
          borderRadius: 10, zIndex: 100, fontWeight: 500, whiteSpace: 'nowrap',
        }}>{toast}</div>
      )}
